...
exports.projectList = function(req, res) {
	...
	logger2.debug(user_id + ' call projectList');
    // show your own project and the project shared to you
	Project.find({
	    $or:[{
	            email: user_id
	        }, {
	        ...
	        console.log('[projectList] '+projs.length);
	        var jsonTsprojs = {
			    timeLib: timeLib, // time format utility functions passed to ejs template
			    projs: projs,
			    user_id: user_id,
			    editprojectuser: [],// an array to save different proj settings
			    accessBlocked: false,
		    };
...
exports.authority = function(req, res) {
	...
	session.checkLoginbySess(req, res);
	logger2.debug(user_id + ' call authority');

	var permission = session.getSessofPermission(req);
	// check the permission of project sharing management
        Project.findById(req.params.id, function(err, proj) {
            console.log('[authority] id '+req.params.id);
            console.log('[authority] proj info');
            console.log('[authority] '+proj);
            if (err) return callback(err);

            // find the index of current user's permission settings through proj_share[]. 2017.8.17 Tony
            if (user_id == proj.email) var index = 0;
            else index = proj.proj_share.indexOf(user_id);
            session.setSessbyPermission(req, proj.permission[index]); // 2017.8.17 Tony

            if (proj.permission[index][0]) {
                var jsonProj = {
                    proj: proj
                };
                render.render(req, res, 'authority', 'Project Share List', jsonProj);
            } else {
	...
	Project.find({
		...
		if (err) return callback(err);
			...
			Project.findById(projID, function(err, proj) {
				...
				projInfo['proj_share'] = proj['proj_share'];  //test by sam
 ...
 */
 exports.shareProject = function(req, res) {
	if (session.checkLoginbySessBool(req)) {
		...
		Project.findById(projID, function(err, proj) {
		    var projShare = proj['proj_share'];
            try{
                if(err) res.end(err.toString());
                else if(projShare.indexOf(shareEmail)>0){
                    // find the same email in projShare
                     res.end('repeat email');
                }else{
                    //not find the same email
                    projShare.push(shareEmail);
                    proj['proj_share'] = projShare;
                    // add permission
                    var permission;
                    switch(position) {
                        case 'ProjectOwner' :
                    ...
                    proj['permission'].push(permission);
                    proj.save(function(err, proj) {
                        if (err) res.end(err.toString());
                        else res.end('success');
                    });
...
/**
 * Delete Sharing project in proj_share and permission and update it.
 */
exports.shareProjDestroy = function(req, res) {
	if (session.checkLoginbySessBool(req)) {

        let projID = req.params.id;
        let delProjEmail = req.body.delProjEmail;

		Project.findById(projID, function(err, proj) {
		    if (err)  res.end(err.toString());
            let index = proj['proj_share'].indexOf(delProjEmail);
		    if(index>-1){
			    proj['proj_share'].splice(index,1);
			    proj['permission'].splice(index,1);
			}
			proj.save(function(err, proj) {
			    if (err) res.end(err.toString());
			    res.end('success');
...
exports.editauthority = function(req,res){
    const  authodCount = 3;    //The count of authods .
	if (session.checkLoginbySessBool(req)) {

		let projID = req.body.projID;
		let shareEmail = req.body.shareEmail;    //shareEmail is an array about Email in table .
        let shareUserAuthod = req.body.shareUserAuthod;    //shareUserAuthod is an array about authod in table .
		Project.findById(projID, function(err, proj) {

		    let projShare = proj.proj_share;
            if(err)    res.end(err.toString());
		    else if(projShare != ''){    // if the property of table about proj_share isn`t null .
                let j=0;
                for(let i=0 ;i<shareEmail.length;i++){
                    proj.permission.pull();    // Delete the property project.permission
                    proj.permission[i] = [Number(shareUserAuthod[j]),Number(shareUserAuthod[j+1]),Number(shareUserAuthod[j+2])];    //It`s groups every three .
                    j+=authodCount;    // authodCount is number '3' .
                    proj.permission.push();
                }
                proj.save(function(err, content) {
				    if (err) return callback(err);
				    else{
				        res.end('success');
				    }
