...
def main():
    """ Get the report dict from parser and put it in spiratest server by api.
    ...
    report = PythonHtmlParser.PythonHtmlParser(reportFileName)
    result = report.parsePythonUnitTestReport()

    start_timestamp = time.mktime(time.strptime(result['start_time'], "%Y-%m-%d %H:%M:%S"))
    duration_time_array = result['duration'].split(':')
    end_timestamp = start_timestamp + float(duration_time_array[0]) * 3600 + float(duration_time_array[1]) * 60 + float(duration_time_array[2])

    spira_test_api = spira_test_v4_0.spira_test(projectID, projectName, host, port, username, apiKey)
    #spira_test_case_folders represents all the testcase folders in the project
    ...
    #if TaaS_test_case_folder is None, it means that user doesn't create the route of testcase folder
    ...
    for test_case_folder in result['test_suites']:
        #check if there is any folder created, spira_test_case_folders have to be updated while testcase is created
        check_update = [False]
        #record the path while finding target testcase folder
        ...
        if(check_update[0]):
            spira_test_case_folders = spira_test_api.getTestCaseFolders()
        #if folder is None, it means autocreate is closed and user doesn't create the route of testcase folder
        if target_testcase_folder is not None:
            for test_case in test_case_folder['test_cases']:
                #change the name of testcase before finding it
                if regexPattern != "":
                    try:
